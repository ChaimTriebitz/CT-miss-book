...
function query(filterBy) {
	console.log('filter from service', filterBy)
	...
	if (filterBy) {
		console.log('filter from service', filterBy)
		let { byTitle, byMinPrice, byMaxPrice } = filterBy
		console.log('filterBy from service', filterBy)
		if (!byMinPrice) byMinPrice = 0
		if (!byMaxPrice) byMaxPrice = Infinity
		books = books.filter(
			(book) =>
				book.title.includes(byTitle) &&
				book.listPrice.amount >= byMinPrice &&
				book.listPrice.amount<= byMaxPrice
		)
	}
...
function getBookTitles() {
	return gBooks.map((book) => book.title)
}
